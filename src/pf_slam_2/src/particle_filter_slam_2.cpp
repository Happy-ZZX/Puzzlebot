...
void ParticleFilter::ImageCallback(const sensor_msgs::ImageConstPtr &img_ptr)
{
    ...
    ROS_INFO_STREAM("Number of Picture" << num_pic << std::endl << "NUmber of Update" << num_update << std::endl);
}
...
void ParticleFilter::addEncoder(const double &enl, const double &enr)
{
    ...
    /***** update covariance *****/
    Eigen::Matrix3d Hx;
    Hx << 1.0, 0.0, -delta_s * sin_tmp_th,
        0.0, 1.0, delta_s * cos_tmp_th,
        0.0, 0.0, 1.0;
    sigma_ = Hx * sigma_ * Hx.transpose() + encoder_noise;
    // ROS_INFO_STREAM("Encoder noise:" << sigma_ << std::endl);
    ...
    if (obs_.size() > 0)
    {
        ...
        for (int j = 0; j < obs_.size() ; j++)
        {
            ...
            else
            {
                if (Distance <= 0.2)
                {
                    ROS_INFO_STREAM("Not satisfy the Distance Window" <<  std::endl);
                    return;
                }

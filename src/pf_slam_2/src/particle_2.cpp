#include "pf_slam_2/particle_2.h"
...
void Particle:: sample(const Eigen::Matrix3d &EncoderNoise)
{
    // arma::Col<double> Mean(3, arma::fill::zeros);
    // arma::Mat<double> Cov(3, 3);

    // Cov(0,0) = EncoderNoise(0,0);
    // Cov(0,1) = EncoderNoise(0,1);
    // Cov(0,2) = EncoderNoise(0,2);
    // Cov(1,0) = EncoderNoise(1,0);
    // Cov(1,1) = EncoderNoise(1,1);
    // Cov(1,2) = EncoderNoise(1,2);
    // Cov(2,0) = EncoderNoise(2,0);
    // Cov(2,1) = EncoderNoise(2,1);
    // Cov(2,2) = EncoderNoise(2,2);
    // arma::Col<double> Delta = arma::mvnrnd(Mean, Cov);

    // pose(0) += Delta(0);
    // pose(1) += Delta(1);
    // pose(2) += Delta(2);
    // ROS_ERROR_STREAM("Delta Sample" << Delta(0) << std::endl << Delta(1)<< std::endl << Delta(2) << std::endl);

    std::random_device rd;
    std::mt19937 generator(rd());
    std::normal_distribution<double> dist(0.0, EncoderNoise(0,0));
    pose(0) += dist(generator);
    pose(0) -= dist(generator);
    std::mt19937 generator2(rd());
    std::normal_distribution<double> dist2(0.0, EncoderNoise(1,1));
    pose(1) += dist2(generator2);
    pose(1) -= dist2(generator2);
    std::mt19937 generator3(rd());
    std::normal_distribution<double> dist3(0.0, EncoderNoise(2,2));
    pose(2) += dist3(generator3) ;
    pose(2) -= dist3(generator3) ;
    // ROS_ERROR_STREAM ("radom number is " << dist(generator) << std::endl);
    normAngle(pose(2));
}
